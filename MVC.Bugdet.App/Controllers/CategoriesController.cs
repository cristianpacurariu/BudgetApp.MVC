...
namespace MVC.Bugdet.App.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        public async Task<ActionResult> Create(OperationTypeViewModel vm)
        {
            if (!ModelState.IsValid)
            {
                return View(vm);
            }
            OperationTypeDto toAdd = Mapper.Map<OperationTypeViewModel, OperationTypeDto>(vm);
            await _operationTypeRepo.Add(toAdd);
        ...
        public async Task<ActionResult> Edit(OperationTypeViewModel vm)
        {
            if (ModelState.IsValid)
            {
                OperationTypeDto toEdit = Mapper.Map<OperationTypeViewModel, OperationTypeDto>(vm);
                await _operationTypeRepo.Update(toEdit);
                return RedirectToAction("Index");
            }
            return View(vm);
        }
